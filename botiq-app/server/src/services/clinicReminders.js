...
export async function scheduleRecallReminder(tenantId, patientId, patientName) {
...
export async function processDueReminders() {
  ...
  for (const row of due) {
    ...
    try {
      ...
      await run(`UPDATE reminders SET status = 'sent' WHERE id = ?`, [row.id]);

      if (row.appointment_id && row.reminder_type?.startsWith('appointment_')) {
        await run(`UPDATE appointments SET reminder_sent = true WHERE id = ?`, [row.appointment_id]);
      }
    } catch (err) {
