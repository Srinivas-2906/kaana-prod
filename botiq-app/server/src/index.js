...
app.get('/health', (_req, res) => {
  res.json({ ok: true, provider: activeProvider, database: 'postgres' });
});
...
// API info root (only when not serving the inbox SPA)
...
async function handleMetaWebhook(body) {
  ...
  for (const entry of body.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      const tenant = await getTenantByWhatsAppPhoneId(phoneNumberId);
...
async function bootstrap() {
  ...
  app.listen(PORT, () => {
    console.log('');
    console.log('🏠 BotIQ — Unified Server (Postgres)');
