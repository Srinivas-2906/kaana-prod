import { getClient, getRequestTenantId, runWithTenantAsync, checkTenantLimits, incrementUsage, getTenantById, ensureTenantCached } from './tenantContext.js';
...
async function botSay(phone, text) {
  await prepareReply();
  await addMessage(phone, 'bot', text);
  await sendText(phone, text);
  await incrementUsage(getRequestTenantId(), 'bot_replies');
  await delay(150);
}
...
async function showResults(phone, { reset = false } = {}) {
  ...
  await prepareReply();
  await addMessage(phone, 'bot', countMsg);
  await sendText(phone, countMsg);
  await delay(150);

  for (const card of items) {
    await addMessage(phone, 'bot', `${card.title} · ${card.price} · ${card.location} · ${card.bhk}`, { propertyCards: [card] });
...
async function escalateToAgent(phone, reason = 'Customer requested agent') {
  ...
  await updateConversation(phone, { status: 'agent', unread: 1 });
  await botSay(
    phone,
...
async function confirmBooking(phone, contactText) {
  ...
  await setConversationName(phone, name);
  ...
  await updateConversation(phone, { status: 'resolved', stats: { resolution: 'Automated ✓', timeToBook: '4 minutes' } });
...
export async function handleIncomingMessage(phone, rawBody, buttonPayload, incomingMessageId, tenantId = getRequestTenantId()) {
  await ensureTenantCached(tenantId);
...
async function handleIncomingMessageInner(phone, rawBody, buttonPayload, incomingMessageId, tenantId) {
  ...
  await addMessage(phone, 'user', body || buttonPayload, {}, tenantId);

  if (RESTART_RE.test(body) && !buttonPayload) {
    resetSession(phone);
    await updateConversation(phone, { status: 'bot', unread: 0 });
