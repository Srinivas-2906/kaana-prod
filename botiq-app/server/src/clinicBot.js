...
function parseServiceSelection(body, tenantId) {
  ...
  const items = await getCatalogItems(tenantId);
...
async function startBookingWithService(phone, serviceTitle, tenant, messageId) {
  ...
  const existing = await getPatientByPhone(phone, tenant.id);
  ...
  return runWithTenantAsync(tenant.id, async () => {
    ...
    if (phase === 'menu') {
      ...
      if (/talk|staff|speak|team|call/i.test(choice)) {
        ...
        await getOrCreatePatient({ phone, name: session.name, tenantId: tenant.id, conversationId: convIdForPhone(phone) });
    ...
    if (phase === 'name') {
      ...
      await setConversationName(phone, name, tenant.id);
      await sendOptions(phone, {
    ...
    if (phase === 'date') {
      ...
      const slots = await getAvailableSlots(tenant.id, dateStr);
