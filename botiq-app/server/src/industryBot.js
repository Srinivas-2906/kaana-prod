...
async function botReply(phone, text, messageId) {
  ...
  await incrementUsage(getRequestTenantId());
}
...
async function sendCatalogCard(phone, item) {
  ...
  await addMessage(phone, 'bot', `${item.title} — ${item.price}`, { card: true });
  await incrementUsage(getRequestTenantId());
}
...
export async function handleIndustryMessage(phone, body, buttonPayload, messageId, tenant) {
  return runWithTenantAsync(tenant.id, async () => {
    const limits = await checkTenantLimits(tenant.id);
    if (!limits.ok) {
    ...
    await addMessage(phone, 'user', body || buttonPayload || '');
    ...
    if (phase === 'menu') {
      ...
      if (/talk|call|agent|staff|support|counsellor|expert|trainer|sales|stylist/i.test(choice)) {
        ...
        await createLead({ phone, name: session.name || 'WhatsApp Lead', stage: 'agent', note: choice, tenantId: tenant.id });
        await updateConversation(phone, { status: 'agent' }, tenant.id);
    ...
    if (phase === 'book_time') {
      ...
      const items = await getCatalogItems(tenant.id);
      const item = items[0];
      await createLead({
        phone,
    ...
    if (phase === 'after_catalog') {
      ...
      if (choice?.includes('Talk')) {
        patchSession(phone, { phase: 'agent' });
        await createLead({ phone, stage: 'agent', tenantId: tenant.id });
        await updateConversation(phone, { status: 'agent' }, tenant.id);
    ...
    if (phase === 'track') {
      await createLead({ phone, stage: 'new', note: `Tracking: ${body}`, tenantId: tenant.id });
...
export async function startIndustryConversation(phone, tenant) {
  ...
  await addMessage(phone, 'bot', welcome);
  await sendOptions(phone, { text: 'Choose an option:', options: menu });
}
